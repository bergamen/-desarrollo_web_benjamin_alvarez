...
const poblar_regiones = () => {
    ...
    for(const regiones in region_comuna["regiones"]){
        ...
        option.value = region_comuna["regiones"][regiones]["nombre"];
...
const update_comunas = () => {
  ...
  if (region_comuna["regiones"][region_000]) {
      region_comuna["regiones"][region_000]["comunas"].forEach(comuna => {
          let option = document.createElement("option");
...
const poblar_contactos = () => {
    let contSelect = document.getElementById("select-contacto");
    for(const contactos in data["contactos"]){
        let option = document.createElement("option");
        option.value = data["contactos"][contactos];
        option.text = data["contactos"][contactos];
        contSelect.appendChild(option);
    }
}

const activate_otras = () => {
    let valor_contacto = document.getElementById("select-contacto").value;
    let bloque = document.getElementById("otros");
    if (valor_contacto=="whatsapp" || !valor_contacto){
        bloque.hidden = true;
    }
    else{
        bloque.hidden = false;
...
document.getElementById("select-contacto").addEventListener("change",activate_otras);
