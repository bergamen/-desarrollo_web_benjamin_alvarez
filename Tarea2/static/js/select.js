...
var region_000 = 0
const poblar_regiones = () => {
    let region = document.getElementById("select-region");

    for(const regiones in region_comuna["regiones"]){
        let option = document.createElement("option");
        option.value = regiones;
        option.text = region_comuna["regiones"][regiones]["nombre"];
...
const update_comunas = () => {
  ...
  comunaSelect.innerHTML = '<option value="">Seleccione una comuna</option>';
    ...
    for(const regiones in region_comuna["regiones"][selectedRegion]["comunas"]){
        let option = document.createElement("option");
        option.value = region_comuna["regiones"][selectedRegion]["comunas"][regiones]["id"];
        option.text = region_comuna["regiones"][selectedRegion]["comunas"][regiones]["nombre"];
        comunaSelect.appendChild(option);
    }  
};

document.getElementById("select-region").addEventListener("change", update_comunas);


// select de la segunda forma

const data = {
    "contactosT":[
    "whatsapp", "telegram", "X",
    "instagram", "tiktok" , "otra"]
};

const poblar_contactos = () => {
    let contSelect0 = document.getElementById("select-contacto-0");
    let contSelect1 = document.getElementById("select-contacto-1");
    let contSelect2 = document.getElementById("select-contacto-2");
    let contSelect3 = document.getElementById("select-contacto-3");
    let contSelect4 = document.getElementById("select-contacto-4");
    for(const contactos in data["contactosT"]){
        let option = document.createElement("option");
        option.value = data["contactosT"][contactos];
        option.text = data["contactosT"][contactos];
        contSelect0.appendChild(option);
    }
    for(const contactos in data["contactosT"]){
        let option = document.createElement("option");
        option.value = data["contactosT"][contactos];
        option.text = data["contactosT"][contactos];
        contSelect1.appendChild(option);
    }
    for(const contactos in data["contactosT"]){
        let option = document.createElement("option");
        option.value = data["contactosT"][contactos];
        option.text = data["contactosT"][contactos];
        contSelect2.appendChild(option);
    }
    for(const contactos in data["contactosT"]){
        let option = document.createElement("option");
        option.value = data["contactosT"][contactos];
        option.text = data["contactosT"][contactos];
        contSelect3.appendChild(option);
    }
    for(const contactos in data["contactosT"]){
        let option = document.createElement("option");
        option.value = data["contactosT"][contactos];
        option.text = data["contactosT"][contactos];
        contSelect4.appendChild(option);
    }
}

//document.getElementById("select-contacto-0").addEventListener("change",activate_otras);

// Tercera forma

const tipos = ["perro","gato"];
const medidas = ["meses","años"];

const poblar_forma = () => {
    let tiposSelect = document.getElementById("select-tipo");
    ...
    let medidaSelect = document.getElementById("select-medida");
    for(const medida in medidas){
        let option = document.createElement("option");
        option.value = medidas[medida][0];
        option.text = medidas[medida];
       medidaSelect.appendChild(option);
    }
}


const set_date = (fecha) => {
    const dia = fecha;
...
window.onload = () => {
  ...
  let fecha_op = document.getElementById("fecha");
  fecha_op.min = fecha.toISOString().slice(0,16)
  fecha_op.value= fecha.toISOString().slice(0,16);
};
