...
def get_last_foto(id):
    session = SessionLocal()
    adops = session.query(Foto.ruta_archivo).filter(Foto.actividad_id==id).first()
    session.close()
    return adops.ruta_archivo
...
def cantidad_tipos():
    session = SessionLocal()
    tabla = session.query(AvisoAdopcion.tipo,func.count(AvisoAdopcion.tipo)).group_by("tipo").all()
    session.close()
    return tabla
def grafico_linea():
    session = SessionLocal()
