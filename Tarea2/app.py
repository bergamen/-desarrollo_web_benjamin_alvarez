...
def principal():
    ...
    for conf in db.get_latest():
        ...
        conf_img = f"uploads/{foto}" if foto else "svg/anonymous.svg"
        ...
        data.append({
            ...
            "comuna":comuna,
            "sector":sector,
            "cantidadEdad":str(conf.cantidad)+" "+str(conf.tipo)+","+str(conf.edad)+" "+str(conf.unidad_medida),
...
def listado(id=None):
    ...
    for conf in tabla:
        ...
        data.append({
            ...
            "comuna": comuna,
            "sector":conf.sector,
            "cantidadEdad": str(conf.cantidad)+" "+conf.tipo.name+","+str(conf.edad)+" "+conf.unidad_medida.name ,
            "nombre":conf.nombre+"\n"+str(contacto),
...
def detalle(id=None):
    ...
    i=0
    for conf in fotos:
        conf_img = f"uploads/{conf.ruta_archivo}" if conf.nombre_archivo else "svg/anonymous.svg"
        data_foto.append({
    ...
    for conf in contactos:
        ...
        data_contacto.append({
            "nombre": str(conf.nombre)+":"+conf.identificador
        })
    ...
    data_aviso = {
        ...
        "email":avisos.email,
        "datos":str(avisos.cantidad) + " " +str(avisos.tipo)+","+str(avisos.edad)+" "+str(avisos.unidad_medida),
...
def adoption():
    if request.method == "POST":
        ...
        # 2. save img as a file
            fotos0.save(os.path.join(app.config["UPLOAD_FOLDER"], img_filename))
            ...
            if(fotos1):
                ...
                # 2. save img as a file
                fotos1.save(os.path.join(app.config["UPLOAD_FOLDER"], img_filename))
            ...
            if(fotos2):
                ...
                # 2. save img as a file
                fotos2.save(os.path.join(app.config["UPLOAD_FOLDER"], img_filename))
            ...
            if(fotos3):
                ...
                # 2. save img as a file
                fotos3.save(os.path.join(app.config["UPLOAD_FOLDER"], img_filename))
            ...
            if(fotos4):
                ...
                # 2. save img as a file
                fotos4.save(os.path.join(app.config["UPLOAD_FOLDER"], img_filename))
...
def get_stats_data():
    ...
    new_data = [{"name":data[0].tipo.perro.name,"y":data[0][1]},{"name":data[1].tipo.gato.name,"y":data[1][1]}]
